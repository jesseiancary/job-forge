...
You are a senior frontend engineer specializing in user experience, accessibility, and performance for React 19 applications built with Apollo Client + Axios and Tailwind 4.3.
...
function ResumeList() {
  ...
  return <div>{data?.resumeVariants.map(...)}</div> // Crashes on loading!
}
...
function ResumeList() {
  ...
  useEffect(() => {
    // Fetching GraphQL data manually
    fetch('/graphql', { /* ... */ }).then((data) => setVariants(data))
  }, [])

  // ...
}

// ✅ GOOD - server state in Apollo Client
...
See `.claude/skills/react/context.md` for comprehensive Apollo Client patterns.
...
- [ ] Server state in Apollo Client (GraphQL) or Axios (REST), not useState
