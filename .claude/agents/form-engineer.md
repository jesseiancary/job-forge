...
2. **Zod validation** before submission (client-side + server-side via Pydantic)
...
7. **Optimistic updates** where appropriate (Apollo Client or Axios)
...
// 3. Component with form state
export function CreateResumeForm({ onSuccess }: Props) {
