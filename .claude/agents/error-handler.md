...
**Custom error format (optional, for richer errors):**

```json
{
  "detail": {
    "code": "VARIANT_IN_USE",
    "message": "Cannot delete variant used by 3 application(s)",
    "applications": [
      {"id": "1", "company": "Stripe"},
      {"id": "2", "company": "Cedar"}
    ]
...
## FastAPI Error Handling (backend/app/exceptions.py)

```python
from fastapi import HTTPException

# Simple error (most common)
raise HTTPException(status_code=400, detail="Cannot delete last resume variant")

# Custom error class (for richer errors)
class AppError(HTTPException):
    def __init__(self, message: str, status_code: int, code: str, details: dict = None):
        super().__init__(
...
raise AppError(
    message="Cannot delete variant used by applications",
    status_code=409,
    code="VARIANT_IN_USE",
    details={"application_count": 3}
)
```

## Global Error Handler (backend/app/main.py)
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    # Log validation error (no sensitive data)
...
# Exception handler for generic exceptions (500)
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    # Development: return error details for debugging
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        content={
            "detail": str(exc),
...
### Apollo Client Error Handling (GraphQL)
...
function ResumeList() {
  const { data, error, refetch } = useQuery(GET_RESUME_VARIANTS, {
    // Apollo Client has built-in retry logic via apollo-link-retry
    // Configure in apollo-client.ts setup
  })

  if (error) {
    // Apollo Client errors have different structure
      ...
      if (errorCode === 'FORBIDDEN') {
        return (
          <div className="text-center py-12">
            <p className="text-gray-500">You don't have permission to view this resource</p>
          </div>
        )
      }
      ...
      if (errorCode === 'NOT_FOUND') {
        return (
          <div className="text-center py-12">
            ...
            <Button onClick={() => navigate('/resumes')}>View all resumes</Button>
          </div>
        )
      }
    }
...
function PersonalInfoPage() {
  ...
  const fetchPersonalInfo = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await api.get('/api/v1/personal-info')
      ...
      // Handle specific error codes
      if (errorCode === 'NOT_FOUND') {
        setError({ message: 'Personal info not found. Please complete your profile.' })
      } else {
        setError(err)
      }
    } finally {
  ...
  if (error) {
    return (
      <ErrorMessage
        ...
        message={error.message || 'An unexpected error occurred'}
        action={<Button onClick={fetchPersonalInfo}>Retry</Button>}
      />
    )
  }
